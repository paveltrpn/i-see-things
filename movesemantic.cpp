...
class move_sem_c {
    private:
        ...
        //    диагностическое сообщение. Поскольку static_assert не обрабатывается во время
        //    выполнения, то стейтменты static_assert могут быть размещены в любом месте кода (даже
    ...
    public:
        ...
        //   Порождённый объект будет "таким же, но не им", т.е. иметь теже данные, но в другой
        //   области памяти.
        ...
        void show_allocated() {
            std::cout << "move_sem_c(this) - " << this << "\n"
                      << "allocated - " << allocated << "\n"
                      << "malloc_usable_size - " << malloc_usable_size(heap.get()) / sizeof(T)
                      << " bytes\n"
                      << "heap place - " << heap.get() << "\n"
                      << std::endl;
        };
